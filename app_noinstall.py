...
------------------------------------------------------
* Kurumsal PC'ye "pip install" gerekmez.
* Uygulamayı Streamlit Cloud veya Hugging Face Spaces'a deploy ederek kullanın.
...
Not: Bu sürüm, kalıcı depolamayı tarayıcıdan indir/yükle akışı ile yönetir.
"""
import io
import uuid
...
def load_from_xlsx(file_bytes: bytes):
    ...
    for name in data.keys():
        if name in x.sheet_names:
            data[name] = pd.read_excel(x, sheet_name=name, engine="openpyxl")
    return data
